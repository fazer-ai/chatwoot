...
RSpec.describe CustomFilterPolicy, type: :policy do
  ...
  # Auris-flavor: managers behave like administrators for elevated
  # operations. AccountUser carries `role: :manager` as a real enum value
  # (see app/models/account_user.rb), so we promote the user to manager
  # here instead of stubbing the predicate.
  let(:manager_context) do
    account_user = manager.account_users.find_by(account: account)
    account_user.update!(role: :manager)
    { user: manager, account: account, account_user: account_user }
  end
  ...
  permissions :index?, :create? do
    ...
    # Auris: managers can list/create custom filters too.
    it { expect(custom_filter_policy).to permit(manager_context, build(:custom_filter)) }
  end
  ...
  permissions :show?, :update?, :destroy? do
    context 'when record is global' do
      ...
      it 'permits managers to update and destroy global filters (Auris elevation)' do
        expect(custom_filter_policy.new(manager_context, global_filter).show?).to be true
        expect(custom_filter_policy.new(manager_context, global_filter).update?).to be true
        expect(custom_filter_policy.new(manager_context, global_filter).destroy?).to be true
      end
    ...
    context 'when record is personal' do
      ...
      it 'denies non-authors, including admins and managers' do
        ...
        expect(custom_filter_policy.new(manager_context, personal_filter).update?).to be false
        expect(custom_filter_policy.new(manager_context, personal_filter).destroy?).to be false
      end
    end
  end
end
