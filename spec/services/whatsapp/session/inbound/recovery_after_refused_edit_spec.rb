...
RSpec.describe 'a recovery caught inside a refused optimistic edit', type: :request do
  ...
  describe 'the record that keeps a recovery answerable' do
    ...
    # An execution that finds another one's claim owns nothing: that other one may still be acting, and
    # ending the record here would take it out from under its retry if its action raises.
    it 'leaves the record to the execution that holds the claim' do
      recover_into('quero um orçamento')
      drain
      row = stored
