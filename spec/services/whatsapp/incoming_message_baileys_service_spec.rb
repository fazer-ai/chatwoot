  ...
  describe '#perform' do
    ...
    context 'when processing messages.upsert event' do
      ...
      context 'when message type is image' do
        ...
        let(:media_stream) do
          media_data = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII='
          StringIO.new(Base64.decode64(media_data))
        end

        it 'creates the message with caption' do
          allow(Down).to receive(:download)
            .with('https://baileys.api/media/msg_123', headers: inbox.channel.api_headers)
            .and_return(media_stream)
