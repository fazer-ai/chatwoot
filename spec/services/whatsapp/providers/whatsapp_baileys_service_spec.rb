...
describe Whatsapp::Providers::WhatsappBaileysService do
  ...
  describe '#send_message' do
    ...
    let(:result_body) { { 'data' => { 'key' => { 'id' => 'msg_123' }, 'messageTimestamp' => 123 } } }
    ...
    context 'when message is a text' do
      ...
      it 'updates the message with content attribute external_created_at' do
        stub_request(:post, request_path)
          .with(
            headers: stub_headers(whatsapp_channel),
            body: {
        ...
        expect(message.reload.content_attributes['external_created_at']).to eq(123)
      end
    end
