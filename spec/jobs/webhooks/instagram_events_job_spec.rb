  ...
  describe '#perform' do
    ...
    context 'when handling messaging events for Instagram via Instagram login' do
      ...
      before do
        ...
        stub_request(:get, %r{https://graph\.instagram\.com/#{Channel::Instagram.api_version}/Sender-id-.*\?.*})
          .to_return(
            status: 200,
            body: proc { |request|
      ...
      it 'does not create contact or messages when Instagram API call fails' do
        ...
        stub_request(:get, %r{https://graph\.instagram\.com/#{Channel::Instagram.api_version}/.*\?.*})
