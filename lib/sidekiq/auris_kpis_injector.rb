...
class Sidekiq::AurisKpisInjector
  ...
  # (no jobs ran → no failures to count). Two decimal places so the
  # operator can see when the rate is close to 100% but not exactly
  # there (e.g. `99,96%` instead of `100%`). Brazilian decimal comma.
